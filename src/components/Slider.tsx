...
export default function Slider({ type }: { type: TypesMovies }) {
  const navigate = useNavigate();
  const bigMovieMatch: PathMatch<string> | null = useMatch(
    `/moonflix/movies/${type}/:movieId`
  );
  const { data } = useQuery<IGetMovieResult>(['movies', type], () =>
    getMovies(type)
  );
  ...
  const onBoxClicked = (movieId: number, type: string) => {
    navigate(`/moonflix/movies/${type}/${movieId}`);
  };

  const onOverlayClick = () => navigate('/moonflix');
  ...
  return (
    <>
      <Sliders>
        <Title>
          {type === TypesMovies.now_playing
            ? 'now playing'
            : type === TypesMovies.top_rated
            ? 'top rated'
            : type}
        </Title>
