...
export default function SearchSlider({ keyword }: { keyword: string }) {
  const navigate = useNavigate();
  const bigMovieMatch: PathMatch<string> | null = useMatch(
    '/moonflix/search/:searchId'
  );
  ...
  const { data } = useQuery<IGetSearchResult>(['search', keyword], () =>
    getSearch(keyword || '')
  );
  ...
  const onBoxClicked = (searchId: number) => {
    navigate(`/moonflix/search/${searchId}`);
  };

  const onOverlayClick = () => navigate(`/moonflix/search?query=${keyword}`);

  const clickedMovie =
    bigMovieMatch?.params.searchId &&
    data?.results.find(
      (search) => search.id === +bigMovieMatch.params.searchId!
    );
